...
static void audio_packet_reset_fields(AudioPacket *pkt)
{
    if (!pkt) {
        return;
    }
...
static void audio_queue_reset_state(AudioQueue *q)
{
    if (!q) {
        return;
    }
...
void audio_packet_free(AudioPacket *pkt)
{
    ...
    audio_packet_reset_fields(pkt);
}

static void audio_queue_free_slots(AudioQueue *q)
{
    if (!q || !q->slots) {
        return;
    }
...
{
    if (!q || capacity <= 0 || chunk_capacity == 0) {
        return -1;
    }
    ...
    if (!q->slots) {
        perror("calloc audio queue slots");
        return -1;
    }
    ...
    for (int i = 0; i < capacity; i++) {
        if (audio_packet_init(&q->slots[i], chunk_capacity) < 0) {
            audio_queue_free_slots(q);
            audio_queue_reset_state(q);
            return -1;
        }
    }
