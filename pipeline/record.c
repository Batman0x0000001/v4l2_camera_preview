...
static int64_t record_timestamp_us_to_pts(uint64_t delta_us, AVRational time_base) {
    return av_rescale_q((int64_t)delta_us, (AVRational){1, 1000000}, time_base);
}

static int record_is_supported_video_input_pixfmt(uint32_t pixfmt) {
    return pixfmt == V4L2_PIX_FMT_YUYV;
}

static void record_stop_queues(AppState *app) {
    if (!app) {
        return;
    }
...
void record_state_init(AppState *app, const char *output_dir, int fps) {
    ...
    snprintf(app->record.output_dir, sizeof(app->record.output_dir), "%s", output_dir ? output_dir : "recordings");
    app->record.fps = (fps > 0) ? fps : 30;
...
static int record_init_common_objects(AppState *app) {
    ...
    if (!app->record.mutex) {
        LOG_ERROR("SDL_CreateMutex(record) failed: %s", SDL_GetError());
        return -1;
    }
    return 0;
}
...
static int record_init_queue(AppState *app) {
    ...
    if (frame_queue_init(&app->record.queue, 8, frame_bytes, app->width, app->height,
                         (int)app->bytesperline, app->pixfmt) < 0) {
        LOG_ERROR("frame_queue_init(record) failed");
        return -1;
    }
    return 0;
}
...
static int record_init_video_encoder(AppState *app) {
    ...
    if (!app->record.encoder) {
        LOG_ERROR("avcodec_find_encoder(record video) failed");
        return -1;
    }
    app->record.enc_ctx = avcodec_alloc_context3(app->record.encoder);
    if (!app->record.enc_ctx) {
        LOG_ERROR("avcodec_alloc_context3(record video) failed");
        return -1;
    }
    ...
    if (ret < 0) {
        LOG_ERROR("avcodec_open2(record video) failed");
        return -1;
    }
    return 0;
}
...
static int record_init_audio_encoder(AppState *app) {
    ...
    if (!app->record.audio_encoder) {
        LOG_ERROR("avcodec_find_encoder(record audio) failed");
        return -1;
    }
    app->record.audio_enc_ctx = avcodec_alloc_context3(app->record.audio_encoder);
    if (!app->record.audio_enc_ctx) {
        LOG_ERROR("avcodec_alloc_context3(record audio) failed");
        return -1;
    }
    app->record.audio_enc_ctx->codec_type = AVMEDIA_TYPE_AUDIO;
    app->record.audio_enc_ctx->sample_fmt = pick_audio_sample_fmt(app->record.audio_encoder);
    app->record.audio_enc_ctx->sample_rate = pick_audio_sample_rate(app->record.audio_encoder,
        (int)app->audio.sample_rate);
    app->record.audio_enc_ctx->time_base = (AVRational){1, app->record.audio_enc_ctx->sample_rate};
    app->record.audio_enc_ctx->bit_rate = 128000;
    app->record.audio_enc_ctx->profile = FF_PROFILE_AAC_LOW;
    if (pick_audio_ch_layout(app->record.audio_encoder, app->audio.channels, &enc_layout) < 0) {
        LOG_ERROR("pick_audio_ch_layout(record audio) failed");
        return -1;
    }
    ret = av_channel_layout_copy(&app->record.audio_enc_ctx->ch_layout, &enc_layout);
    av_channel_layout_uninit(&enc_layout);
    if (ret < 0) {
        LOG_ERROR("av_channel_layout_copy(record audio) failed");
        return -1;
    }
    ret = avcodec_open2(app->record.audio_enc_ctx, app->record.audio_encoder, NULL);
    if (ret < 0) {
        LOG_ERROR("avcodec_open2(record audio) failed");
        return -1;
    }
    return 0;
}
...
static int record_init_video_buffers(AppState *app) {
    int ret;
    app->record.sws_ctx = sws_getContext(app->width, app->height, AV_PIX_FMT_YUYV422,
                                          app->width, app->height, AV_PIX_FMT_YUV420P,
                                          SWS_BILINEAR, NULL, NULL, NULL);
    if (!app->record.sws_ctx) {
        LOG_ERROR("sws_getContext(record video) failed");
        return -1;
    }
    app->record.yuv_frame = av_frame_alloc();
    if (!app->record.yuv_frame) {
        LOG_ERROR("av_frame_alloc(record video) failed");
        return -1;
    }
    ...
    if (ret < 0) {
        LOG_ERROR("av_frame_get_buffer(record video) failed");
        return -1;
    }
    app->record.pkt = av_packet_alloc();
    if (!app->record.pkt) {
        LOG_ERROR("av_packet_alloc(record video) failed");
        return -1;
    }
    return 0;
}
...
static int record_init_audio_buffers(AppState *app) {
    ...
    if (ret < 0 || !app->record.swr_ctx) {
        LOG_ERROR("swr_alloc_set_opts2(record audio) failed");
        return -1;
    }
    ret = swr_init(app->record.swr_ctx);
    if (ret < 0) {
        LOG_ERROR("swr_init(record audio) failed");
        return -1;
    }
    app->record.audio_frame = av_frame_alloc();
    if (!app->record.audio_frame) {
        LOG_ERROR("av_frame_alloc(record audio) failed");
        return -1;
    }
    ...
    ret = av_channel_layout_copy(&app->record.audio_frame->ch_layout,
                                  &app->record.audio_enc_ctx->ch_layout);
    if (ret < 0) {
        LOG_ERROR("av_channel_layout_copy(record audio frame) failed");
        return -1;
    }
    ret = av_frame_get_buffer(app->record.audio_frame, 0);
    if (ret < 0) {
        LOG_ERROR("av_frame_get_buffer(record audio) failed");
        return -1;
    }
    app->record.audio_pkt = av_packet_alloc();
    if (!app->record.audio_pkt) {
        LOG_ERROR("av_packet_alloc(record audio) failed");
        return -1;
    }
    ...
    if (!app->record.audio_fifo) {
        LOG_ERROR("av_audio_fifo_alloc(record audio) failed");
        return -1;
    }
    return 0;
}
...
static int record_init_output(AppState *app) {
    int ret;
    ret = avformat_alloc_output_context2(&app->record.ofmt_ctx, NULL, "mp4",
                                          app->record.active_output_path);
    if (ret < 0 || !app->record.ofmt_ctx) {
        LOG_ERROR("avformat_alloc_output_context2(record) failed");
        return -1;
    }
    app->record.video_st = avformat_new_stream(app->record.ofmt_ctx, NULL);
    if (!app->record.video_st) {
        LOG_ERROR("avformat_new_stream(record video) failed");
        return -1;
    }
    ...
    if (ret < 0) {
        LOG_ERROR("avcodec_parameters_from_context(record video) failed");
        return -1;
    }
    app->record.audio_st = avformat_new_stream(app->record.ofmt_ctx, NULL);
    if (!app->record.audio_st) {
        LOG_ERROR("avformat_new_stream(record audio) failed");
        return -1;
    }
    ...
    if (ret < 0) {
        LOG_ERROR("avcodec_parameters_from_context(record audio) failed");
        return -1;
    }
    if (!(app->record.ofmt_ctx->oformat->flags & AVFMT_NOFILE)) {
        ret = avio_open2(&app->record.ofmt_ctx->pb, app->record.active_output_path,
                         AVIO_FLAG_WRITE, NULL, NULL);
        if (ret < 0) {
            LOG_ERROR("avio_open2(record) failed");
            return -1;
        }
    }
    ret = avformat_write_header(app->record.ofmt_ctx, NULL);
    if (ret < 0) {
        LOG_ERROR("avformat_write_header(record) failed");
...
static void record_finish_session_output(AppState *app) {
    if (!app || !app->record.ofmt_ctx) {
        return;
    }
    /*
     * 只要写头成功并且输出上下文仍存在，就尝试正常写尾。
     * mp4 特别依赖 trailer 收尾。
     */
    av_write_trailer(app->record.ofmt_ctx);
    if (!(app->record.ofmt_ctx->oformat->flags & AVFMT_NOFILE) && app->record.ofmt_ctx->pb) {
        avio_closep(&app->record.ofmt_ctx->pb);
    }
}
...
static void record_close_session_resources(AppState *app) {
    ...
    app->record.encoder = NULL;
    app->record.audio_encoder = NULL;
}
...
static int record_write_encoded_packets(AppState *app, AVCodecContext *enc_ctx, AVPacket *pkt,
                                         AVStream *st, const char *tag) {
    int ret;
    while (1) {
        ...
        if (pkt->pts == AV_NOPTS_VALUE || pkt->dts == AV_NOPTS_VALUE) {
            LOG_WARN("record packet has invalid timestamps (%s): pts=%lld dts=%lld",
                     tag ? tag : "unknown", (long long)pkt->pts, (long long)pkt->dts);
            av_packet_unref(pkt);
            continue;
        }
...
static int record_feed_audio_fifo(AppState *app, const AudioPacket *pkt) {
    ...
    dst_nb_samples = av_rescale_rnd(
        swr_get_delay(app->record.swr_ctx, (int)app->audio.sample_rate) + (int64_t)pkt->meta.frames,
        app->record.audio_enc_ctx->sample_rate,
        (int)app->audio.sample_rate,
        AV_ROUND_UP);
    if (dst_nb_samples <= 0) {
    ...
    ret = av_samples_alloc_array_and_samples(&converted_data, NULL,
                                              app->record.audio_enc_ctx->ch_layout.nb_channels,
                                              dst_nb_samples,
                                              app->record.audio_enc_ctx->sample_fmt, 0);
    if (ret < 0) {
        return -1;
    }
    in_data[0] = pkt->data;
    converted_samples = swr_convert(app->record.swr_ctx, converted_data, dst_nb_samples,
                                     in_data, (int)pkt->meta.frames);
    ...
    ret = av_audio_fifo_write(app->record.audio_fifo, (void **)converted_data, converted_samples);
    av_freep(&converted_data[0]);
    av_freep(&converted_data);
...
static int record_encode_audio_from_fifo(AppState *app) {
    int ret;
    int frame_size;
    if (!app || !app->record.audio_fifo || !app->record.audio_enc_ctx) {
        return -1;
    }
    ...
    while (av_audio_fifo_size(app->record.audio_fifo) >= frame_size) {
        ...
        ret = av_audio_fifo_read(app->record.audio_fifo,
                                  (void **)app->record.audio_frame->data, frame_size);
        if (ret < frame_size) {
            return -1;
        }
        ...
        ret = record_write_encoded_packets(app, app->record.audio_enc_ctx,
                                            app->record.audio_pkt, app->record.audio_st,
                                            "record audio");
        if (ret < 0) {
            return -1;
        }
...
static int64_t record_compute_audio_chunk_start_pts(AppState *app, const AudioPacket *pkt) {
    ...
    if (app->record.audio_anchor_capture_time_us < app->record.media_base_timestamp_us) {
        ...
        delta_us = app->record.audio_anchor_capture_time_us - app->record.media_base_timestamp_us;
    }
    anchor_pts = record_timestamp_us_to_pts(delta_us, app->record.audio_enc_ctx->time_base);
    frame_offset = (int64_t)(pkt->meta.first_frame_index - app->record.audio_anchor_first_frame_index);
...
static int record_consume_audio_packet(AppState *app, const AudioPacket *pkt) {
    ...
    if (!app || !pkt || !pkt->data || !app->record.session_active) {
        return 0;
    }
    if (!app->record.mutex) {
        return -1;
    ...
    if (app->record.audio_next_pts == AV_NOPTS_VALUE) {
        ...
        expected_pts = app->record.audio_next_pts + av_audio_fifo_size(app->record.audio_fifo);
        if (llabs(chunk_start_pts - expected_pts) > app->record.audio_enc_ctx->frame_size) {
            LOG_WARN("record audio start drift: chunk_start=%lld expected=%lld diff=%lld",
                     (long long)chunk_start_pts, (long long)expected_pts,
...
static int record_consume_video_packet(AppState *app, const FramePacket *pkt) {
    ...
    if (!record_is_supported_video_input_pixfmt(pkt->pixfmt)) {
        LOG_ERROR("unsupported record packet pixfmt: 0x%x", pkt->pixfmt);
        return -1;
    }
    if (pkt->stride <= 0) {
        LOG_ERROR("invalid record packet stride: %d", pkt->stride);
        return -1;
    }
    if (pkt->width != app->width || pkt->height != app->height) {
        LOG_ERROR("record packet size mismatch: pkt=%dx%d app=%dx%d",
                  pkt->width, pkt->height, app->width, app->height);
        return -1;
    }
    SDL_LockMutex(app->record.mutex);
    record_set_media_base_if_needed(app, pkt->meta.capture_time_us);
    ...
    pts_raw = record_timestamp_us_to_pts(delta_us, app->record.enc_ctx->time_base);
    pts = pts_raw;
    if (app->record.last_input_pts != AV_NOPTS_VALUE && pts_raw <= app->record.last_input_pts) {
        LOG_WARN("record video pts adjusted for monotonicity: raw=%lld last=%lld adjusted=%lld",
                 (long long)pts_raw, (long long)app->record.last_input_pts,
    ...
    sws_scale(app->record.sws_ctx, src_slices, src_stride, 0, pkt->height,
              app->record.yuv_frame->data, app->record.yuv_frame->linesize);
    ...
    ret = record_write_encoded_packets(app, app->record.enc_ctx, app->record.pkt,
                                        app->record.video_st, "record video");
    if (ret < 0) {
        SDL_UnlockMutex(app->record.mutex);
        return -1;
    }
...
static int record_flush_video_encoder(AppState *app) {
    int ret;
    if (!app || !app->record.enc_ctx || !app->record.ofmt_ctx) {
        return 0;
    }
    ...
    return record_write_encoded_packets(app, app->record.enc_ctx, app->record.pkt,
                                         app->record.video_st, "record video flush");
}
...
static int record_flush_audio_encoder(AppState *app) {
    ...
    int remaining;
    int to_copy;
    if (!app || !app->record.audio_enc_ctx || !app->record.ofmt_ctx) {
        return 0;
    }
    frame_size = app->record.audio_enc_ctx->frame_size;
    while (app->record.audio_fifo && av_audio_fifo_size(app->record.audio_fifo) > 0) {
        ...
        ret = av_samples_set_silence(app->record.audio_frame->data, 0, frame_size,
                                      app->record.audio_enc_ctx->ch_layout.nb_channels,
                                      app->record.audio_enc_ctx->sample_fmt);
        if (ret < 0) {
            return -1;
        ...
        ret = av_audio_fifo_read(app->record.audio_fifo,
                                  (void **)app->record.audio_frame->data, to_copy);
        if (ret < to_copy) {
            return -1;
        }
        app->record.audio_frame->pts = app->record.audio_next_pts;
    ...
    return record_write_encoded_packets(app, app->record.audio_enc_ctx,
                                         app->record.audio_pkt, app->record.audio_st,
                                         "record audio flush");
}
...
static int record_thread_main(void *userdata) {
    ...
    if (frame_packet_init(&video_pkt, app->sizeimage, app->width, app->height,
                          (int)app->bytesperline, app->pixfmt) < 0) {
        LOG_ERROR("frame_packet_init(record worker) failed");
        return -1;
    }
    if (audio_packet_init(&audio_pkt, app->audio.period_buffer_bytes) < 0) {
        frame_packet_free(&video_pkt);
        LOG_ERROR("audio_packet_init(record worker) failed");
        return -1;
    }
    while (1) {
        int did_work = 0;
        ...
        if (!audio_queue_stopped) {
            ...
            for (int i = 0; i < audio_burst; ++i) {
                ...
                audio_ret = audio_queue_pop(&app->record.audio_queue, &audio_pkt, 0);
                if (audio_ret == AUDIO_QUEUE_POP_OK) {
                    ...
                    record_enter_fatal_error(app, "audio_queue_pop(record audio) failed");
                    goto out;
                }
            }
        }
        if (!video_queue_stopped) {
            ...
            if (video_ret == FRAME_QUEUE_POP_OK) {
                ...
                record_enter_fatal_error(app, "frame_queue_pop(record video) failed");
                goto out;
            }
        }
...
int record_session_start(AppState *app) {
    ...
    if (!app || !app->record.enabled || !app->record.mutex) {
        return -1;
    }
    ...
    if (make_record_filename(output_path, sizeof(output_path), app->record.output_dir) < 0) {
    ...
    if (app->record.fatal_error) {
        SDL_UnlockMutex(app->record.mutex);
        LOG_ERROR("record_session_start refused: module is in fatal_error state");
        return -1;
    }
    ...
    snprintf(app->record.active_output_path, sizeof(app->record.active_output_path),
             "%s", output_path);
    record_reset_timeline_state(app);
    app->record.accepting_frames = 0;
    app->record.stopping_session = 0;
...
int record_session_stop(AppState *app) {
    ...
    app->record.accepting_frames = 0;
    snprintf(final_path, sizeof(final_path), "%s", app->record.active_output_path);
    SDL_UnlockMutex(app->record.mutex);
    frame_queue_flush(&app->record.queue);
    audio_queue_flush(&app->record.audio_queue);
    SDL_LockMutex(app->record.mutex);
    ...
    record_finish_session_output(app);
    record_close_session_resources(app);
    app->record.session_active = 0;
    app->record.stopping_session = 0;
    app->record.accepting_frames = 0;
    record_reset_timeline_state(app);
    SDL_UnlockMutex(app->record.mutex);
    LOG_INFO("record session stopped: %s", final_path[0] != '\0' ? final_path : "(unknown)");
    return 0;
}
...
void record_notify_pause(AppState *app) {
    ...
    if (app->record.mutex) {
        ...
        /*
         * 暂停后恢复时，音频重新以新的 chunk 作为锚点继续推导 pts，
         * 避免把暂停前 FIFO 内残留的音频继续拼到暂停后时间线上。
         */
        app->record.have_audio_anchor = 0;
        app->record.audio_next_pts = AV_NOPTS_VALUE;
        SDL_UnlockMutex(app->record.mutex);
